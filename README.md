...
- **The answer is literally the quoted spans, joined.** The shipped answerer is extractive, so an
  ungrounded part number is impossible rather than improbable — it is string arithmetic, not a rate
...
Everything below is measured by a committed code path against a hold-out frozen by a rule that
takes no seed and no score. **Three of the twelve predeclared kill conditions fail, and a fourth
passes vacuously.** All four are reported as such, and the verdict table is generated by running the
graded test rather than typed by hand. See [Where it falls short](#where-it-falls-short).
